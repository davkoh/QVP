function [bhat,full_draw,pomega,time]=QVP_alpha(ysmall,Xsmall,tau,iter,burn,chain_i)
if nargin<6
    chain_i=1;
end
...
X_alpha = zeros(T*Q, Q);
for q = 1:Q
    X_alpha((q-1)*T+1:q*T, q) = 1;
end
X_alpha = sparse(X_alpha);

% Difference matrix for Alpha
H_alpha = speye(1*Q,1*Q) - sparse(1+1:1*Q,1:(Q-1)*1,...
    ones(1,(Q-1)*1),1*Q,1*Q); %% This would be the difference matrix that I need!
...
for i = 1:iter
    ...
    % Level Parameters
    % sample Local
    lam_local = 1./gamrnd(1, 1./( 1./eta1_beta + 0.5*beta0(1:K).^2/nu_global));
    % sample Global
    nu_global = 1/gamrnd( 0.5*(K), 1/( 1/eta2_beta + 0.5*sum(sum(beta0(1:K).^2./lam_local))  ) );
    % sample mixture local
    eta1_beta = 1./gamrnd(1, 1./(1 + 1./lam_local));
    % samplel mixture
    eta2_beta = 1/gamrnd(1, 1/( 1/sqrt(T*Q) + 1/(nu_global) ));%eta2_beta = 1/gamrnd(1, 1/( 1/sqrt(T*Q) + 1/nu_global ));
    ...
    % sample Global
    nu_diff_global(qq) = 1/gamrnd( 0.5*K + 1, 1/( 1/eta2_diff_beta(qq) + 0.5*sum(sum(del_beta(qq*K-K+1:qq*K).^2./lam_diff_local(qq*K-K+1:qq*K))) + 0.5 * (del_alpha(qq))^2  )); %nu_diff_global = 1/gamrnd( 0.5*(K*(Q)), 1/( 1/eta2_diff_beta + 0.5*sum(sum(del_beta.^2./lam_diff_local))  ) );
    % samplel mixture
    ...
    % Sample beta0
    Kbeta0 = (iSigm_beta(1:K,1:K) + diag(1./lam_local*1/nu_global));
    %Cbeta0 = chol(Kbeta0,'lower');
    try
        Cbeta0 = chol(Kbeta0,'lower');
    catch
        Cbeta0 = chol(nearestSPD(full(Kbeta0)),'lower'); %SVD does not support sparse matrix so convert Kbeta to normal first
        Cbeta0 = sparse(Cbeta0); %Convert back to sparse matrix
    end
    %beta_hat0 = Kbeta0\(iSigm_beta(1:K,1:K) * beta(1:K));
    try
        beta_hat0 = Kbeta0\(iSigm_beta(1:K,1:K) * beta(1:K));
    catch
        beta_hat0 = pinv(full(Kbeta0))*(iSigm_beta(1:K,1:K) * beta(1:K)); %Incase Kbeta0 is badly scaled.
    end
    beta0 = beta_hat0 + Cbeta0'\randn(K,1);


    % Sample alpha
    ystar = Y - bigX*beta - mu;
     
    HTH_alpha = H_alpha'*sparse(diag(1./nu_diff_global))*H_alpha;
    Kalpha = (HTH_alpha + X_alpha'*iOm*X_alpha);



    try
        Calpha = chol(Kalpha,'lower');
    catch
        Calpha = chol(nearestSPD(full(Kalpha)),'lower'); %SVD does not support sparse matrix so convert Kalpha to normal first
        Calpha = sparse(Calpha); %Convert back to sparse matrix
    end

    try
        alpha_hat = Kalpha\(HTH_alpha*ones(Q,1)*alpha0 + X_alpha'*iOm*ystar);
    catch
        alpha_hat = pinv(Kalpha\(HTH_alpha*ones(Q,1)*alpha0 + X_alpha'*iOm*ystar)); %Incase Kalpha is badly scaled.
    end
    alpha = alpha_hat + Calpha'\randn(Q,1);




    for j = 1:Q
        bigalpha(j*T-T+1 : j*T) = alpha(j);
    end


    % Sample alpha_0
    Kalpha_0 = ( 1/V_alpha_0 + 1/nu_diff_global(1) );
    alpha_hat_0 = Kalpha_0\(1/nu_diff_global(1)*alpha(1));
    alpha0 = alpha_hat_0 + sqrt(Kalpha_0)*randn;
    
    % Define difference alpha
    del_alpha = [alpha(1) - alpha0;(alpha(2 : end) - alpha(1: end - 1))];
