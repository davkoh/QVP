function [bhat,full_draw,time]=BQR(y,X,quant,iter,nburn,chain_i)
...
if chain_i==1
    ...
    plambda=ones(p,n_q); %% Keep these at ones() for non-informative prior
    ptau=ones(1,n_q); %% Keep these at ones() for non-informative prior
else
    beta = -2 + 4*rand(p,n_q);
